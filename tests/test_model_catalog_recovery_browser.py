"""Run isolated real-browser model catalog; all API responses are synthetic."""
...
def test_pending_model_catalog_recovers_in_real_browser():
    ...
    assert node, "Node is required for the model catalog browser regression"
    result = subprocess.run(
        [node, "tests/model_catalog_recovery_browser.cjs"],
        cwd=root,
        env=env,
        capture_output=True,
        text=True,
