...
def test_toast_dismissal_pauses_on_hover_and_keyboard_focus():
    ...
    # Error toasts have explicit Copy/Dismiss controls. Passive success/info
    # messages must let clicks reach the underlying workflow immediately.
    assert ".toast.show{" in STYLE_CSS
    show_rule = STYLE_CSS[STYLE_CSS.index(".toast.show{"):STYLE_CSS.index("}", STYLE_CSS.index(".toast.show{"))]
    assert "pointer-events:auto" not in show_rule
    assert ".toast.show.error{pointer-events:auto;}" in STYLE_CSS
