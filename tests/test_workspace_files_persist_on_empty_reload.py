...
def _ephemeral_guard_block():
    guard_idx = BOOT_JS.find("(S.session.message_count||0) === 0")
    assert guard_idx > 0, "Empty-session guard block not found in boot IIFE"
    # A stale-activation guard may return earlier. Keep the whole scratch branch.
    block_end = BOOT_JS.find("      // Restore the panel from localStorage", guard_idx)
    assert block_end > guard_idx
    return BOOT_JS[guard_idx:block_end]
