...
def test_actual_worker_dispatch_keywords_match_original_sender(gateway,group,authenticated):
    ...
    if gateway and not group and not authenticated:
        ...
        assert captured['kwargs'].get('sender_identity') is (identity if authenticated else None)
