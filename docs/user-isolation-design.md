...
4. **Admin determination**: helper `identity_is_admin(identity)` in `api/ownership.py`: True when the resolved EffectiveAccess has bootstrap-admin grant source, an explicit owner/admin role, wildcard permissions, or `governance:write`/`governance:admin` permission. A wildcard route allowlist never implies administrative access; each endpoint still requires its feature permission. Navigation and administrative preview metadata use the same predicate. Works in `report_only` and even `off` mode (resolver is mode-independent). Michael, Yaser, Odis resolve admin per the live policy.
