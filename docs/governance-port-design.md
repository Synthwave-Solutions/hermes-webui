...
ROUTE_CATALOG: tuple[RouteRule, ...] = (
    ...
    # terminal + commands (RCE-grade; split off chat:use on 2026-07-19 so
    # chat access no longer implies shell access; see rollout note in section 9)
...
| `tests/test_governance_catalog.py` (B1) | endpoint families map to expected permissions (`GET /api/session` = sessions:read, `POST /api/session/delete` = sessions:write, `POST /api/chat/start` = chat:use, `POST /api/terminal/start` = terminal:use, `GET /api/file` = files:read, `POST /api/file/save` = files:write, `POST /api/crons/run` = cron:run, `GET /api/crons/run` = cron:read, `POST /api/extensions/x/sidecar/y` = plugins:write, `POST /api/kanban/dispatch` = chat:use, `POST /api/gateway/restart` = gateway:restart, `POST /api/settings` = config:write); self routes return None; prefix rules honor segment boundaries (`/api/gitfoo` is unknown); unknown `/api/*` returns None. |
...
- Terminal endpoints originally mapped to `chat:use` (reference PTY precedent). DONE 2026-07-19: they now require the dedicated `terminal:use` permission (`/api/terminal/*` and `/api/commands/exec`), granted to the admin and operator roles in the live policy in the same change (behavior preserving). sw-freelancers and sw-ops remain double-blocked: missing route AND missing permission. Revoking terminal per user is now a policy-values edit, no code change.
