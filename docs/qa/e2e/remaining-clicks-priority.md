...
The acceptance catalog now contains **362 stable stories**, including 16 precise realtime voice stories discovered during implementation. The authoritative full-suite run is `20260908-222640-169443000`; its final counts and source fingerprints are in [execution-summary.json](evidence/execution-summary.json). Use the [story matrix](coverage-plan.csv) for exact assertions and gaps; a passing browser scenario is not certification of an entire historical story or every click.
...
| P1 | Remaining files, skill detail and imports | Actual skill edit/cancel/delete; exact workspace upload bytes; child/hidden-directory navigation; valid/malformed JSON transcript import; copied path/download bytes and a denied private-object probe. File CRUD and chat attachments already have separate evidence. | SKILL-004/005/007, FILE rows, UX-010; append a JSON-import story if the actual workflow differs |
| P1 | Board administration and supported transition depth | Disposable board create/rename; board/list and exact fixture filters; bulk change only selected tasks; dependency add/remove and cycle rejection; block/unblock with reason; dispatcher Preview without launching work. | KAN-001/002/005/008/009/011/013/015 |
...
| P1 | Remaining preference behavior | Long-transcript virtualization, default workspace pane on a new chat, actual session source-type filtering, clipboard and sound/notification states. Search result selection, German DOM translation, independent welcome/suggestion hiding and the actual workspace Todos tab are already covered. | PREF-004/021/023/026/035, SESS-002, CHAT-016 |
| P2 | Recovery, clarification, focus and clipboard | Deterministic clarification request/submit/resume; failed send with retained draft; offline/reconnect and stale-client refresh; exact terminal/log clipboard bytes; keyboard focus return and real terminal resize/output. | CHAT-010/018, UX-004/006/008/009, TERM-003/004/006, LOG-003 |
...
| Skill/file/import lifecycle | `btnEditSkillDetail`, `btnDeleteSkillDetail`, `btnUploadWorkspace`, `btnUpDir`, `btnImportJSON`, `btnCopyPreviewRelPath` | Verify cancel/save/delete, exact file/import/clipboard bytes, parent navigation and a separate unauthorized identity. |
| Board/filter/dependency lifecycle | `btnKanbanCreateBoard`, `btnKanbanViewToggle`, `kanbanAssigneeFilter`, `kanbanOnlyMine`, `kanbanTenantFilter`, `kanbanBulkStatus`, `kanbanDependencyInput`, `btnKanbanPreviewDispatcher` | Seed several distinct tasks; prove exact filtered/selected IDs, persisted changes and cycle/outsider rejection. Dispatcher Preview must not launch a worker. |
| Preferences and recovery | `settingsVirtualizeTranscript`, `settingsWorkspacePanelOpen`, `settingsShowCliSessions`, `settingsShowCronSessions`, `settingsShowWebhookSessions`, `logsCopyAll`, `btnReload` | Assert real transcript/panel/list behavior, exact copied log bytes and recovery after a controlled disconnected request. |
| Remaining navigation affordances | `btnAttach`, `btnWorkspacePanelEdgeToggle`, `railExpandToggle`, `btnAccessRequestsRefresh`, `integrationsRefreshBtn` | Open/close the actual affordance and verify its resulting state. Existing file-input upload evidence does not by itself cover the Attach button's chooser interaction. |

## Product decisions and dependent acceptance

- **Public Share:** the visible action returns404 from `/api/share/create`. Its intended contract—public snapshot or authenticated internal link—remains a product decision. SESS-010 remains failing and SESS-011 cannot test revocation beyond failed creation.
- **Todo → Done:** the UI offers direct completion but returns404. Decide whether direct completion should work or require Ready first. Passing Ready→Done does not resolve this failing visible alternative.
