...
- This RFC does **not** modify `GET /api/sessions/events` (the existing global
  session-list invalidation stream routed at `api/routes.py:12349-12350` and
  implemented by `_handle_session_events_stream()` at `api/routes.py:16185`).
...
proposes. It is routed at `api/routes.py:12349-12350` and implemented by
`_handle_session_events_stream()` at `api/routes.py:16185`. It emits bare
...
`_SSE_HEARTBEAT_INTERVAL_SECONDS = 5` (`api/routes.py:1019-1031`) is the current
...
- `_parse_run_journal_event_id()` (`api/routes.py:15681-15694`) and
  `_parse_run_journal_after_seq()` (`api/routes.py:15696-15709`) parse the replay
  ...
  below, §Reconnect).
- `_runner_event_id()` at `api/routes.py:15773-15780` constructs the event `id`
  field as `stream_id:seq`.
- SSE frames carry their `id:` via the `_sse_with_id()` helper, emitted on the
  live `/api/chat/stream` path at `api/routes.py:15926`, on the runner-observe
  path at `api/routes.py:15819`, and during journal replay at
  `api/routes.py:15729` / `15742`.
- `_replay_run_journal()` reads events by `(session_id, stream_id)` at
  `api/routes.py:15711-15743`.
...
`api/routes.py:15773-15780`. Clients must treat it as an opaque string and must
...
`_replay_run_journal()` path (`api/routes.py:15711-15743`) and fall back to the
...
Phase 1 reuses `_SSE_HEARTBEAT_INTERVAL_SECONDS` (`api/routes.py:1019-1031`) for
