...
#
# The count limit is a secondary safety rail; the primary bound is the estimated
# serialized size of full sessions. Multi-MB sidecars expand substantially after
# JSON parsing, so keeping hundreds resident is unsafe on small self-hosted hosts.
# Precedence for the effective count cap is resolved by get_sessions_cache_max():
#   1. config.yaml  webui.sessions_cache_max
#   2. HERMES_WEBUI_SESSIONS_MAX env var
#   3. DEFAULT_SESSIONS_CACHE_MAX
DEFAULT_SESSIONS_CACHE_MAX = 32
SESSIONS_MAX = _env_int("HERMES_WEBUI_SESSIONS_MAX", DEFAULT_SESSIONS_CACHE_MAX)
DEFAULT_SESSION_CACHE_BYTES = 150 * 1024 * 1024


def get_sessions_cache_max_bytes() -> int:
    """Return the full-session cache byte budget.

    The estimate uses each sidecar's UTF-8 JSON byte length. The environment
    value is intentionally read on each call so tests and supervised processes
    can adjust the budget without rebuilding module globals.
    """
    mb = _env_int("HERMES_WEBUI_SESSION_CACHE_MB", 150)
    return mb * 1024 * 1024
...
_SETTINGS_DEFAULTS = {
    ...
    "bot_name": os.getenv(
        "HERMES_WEBUI_BOT_NAME", "SynthPulse"
