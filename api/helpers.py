...
# Large strings (>16 KB tool dumps) bypass the value-keyed LRU above, yet they
# dominate redaction CPU: a session with a dozen big tool outputs re-ran the
# full ~15-regex combined pass on megabytes of unchanged text for every poll.
# The redactor is pure and deterministic, so memoizing big strings by content
# digest is safe (BLAKE2b-128 collision odds are cryptographically negligible)
# and hashing costs ~1 ms/MB versus ~200 ms/MB for the regex pass. Only the
# redacted OUTPUT is stored, keyed by digest, with a total byte budget and an
# LRU eviction order so a burst of giant payloads cannot balloon RSS.
_REDACT_BIG_CACHE: "OrderedDict[bytes, str]" = OrderedDict()
_REDACT_BIG_CACHE_LOCK = threading.Lock()
_REDACT_BIG_CACHE_MAX_TOTAL_CHARS = 24 * 1024 * 1024
_REDACT_BIG_CACHE_MAX_ENTRY_CHARS = 4 * 1024 * 1024
_REDACT_BIG_CACHE_TOTAL_CHARS = 0


def _redact_big_cached(text: str) -> str:
    global _REDACT_BIG_CACHE_TOTAL_CHARS
    if len(text) > _REDACT_BIG_CACHE_MAX_ENTRY_CHARS:
        return _redact_fn_uncached(text)
    key = hashlib.blake2b(
        text.encode("utf-8", "surrogatepass"), digest_size=16
    ).digest()
    with _REDACT_BIG_CACHE_LOCK:
        cached = _REDACT_BIG_CACHE.get(key)
        if cached is not None:
            _REDACT_BIG_CACHE.move_to_end(key)
            return cached
    result = _redact_fn_uncached(text)
    with _REDACT_BIG_CACHE_LOCK:
        if key not in _REDACT_BIG_CACHE:
            _REDACT_BIG_CACHE[key] = result
            _REDACT_BIG_CACHE_TOTAL_CHARS += len(result)
            while (
                _REDACT_BIG_CACHE_TOTAL_CHARS > _REDACT_BIG_CACHE_MAX_TOTAL_CHARS
                and _REDACT_BIG_CACHE
            ):
                _, evicted = _REDACT_BIG_CACHE.popitem(last=False)
                _REDACT_BIG_CACHE_TOTAL_CHARS -= len(evicted)
    return result


def _redact_fn_cached(text):
    if len(text) > _REDACT_CACHE_MAX_TEXT_LEN:
        return _redact_big_cached(text)
...
def _might_contain_sensitive_text(text: str) -> bool:
    ...
    Deliberately tuple-of-`in` scans rather than one compiled alternation:
    CPython's substring search is a fast C memmem, while a 70-branch regex
    alternation (especially with IGNORECASE) benchmarked 5x SLOWER on real
    session payloads. Do not "optimize" this into a regex without measuring.
    """
    if not isinstance(text, str) or not text:
        return False
...
def _collect_strings(v, parts: list) -> None:
    """Append every string nested inside ``v`` to ``parts`` (same reach as
    ``_redact_value``'s recursion, without allocating any copies)."""
    if isinstance(v, str):
        parts.append(v)
...
def _redact_message_list(messages, *, _enabled: bool | None = None):
    """Redact a transcript list with a per-message clean fast path.

    Per-string prefiltering is call-count bound (~70 substring scans per
    string; a large transcript holds 10k+ nested strings). One joined scan
    per message classifies the whole message; clean messages are returned
    by reference with zero per-string scans and zero dict/list copies.

    Soundness: concatenation preserves substrings, so any marker present in
    an individual string is present in the joined text (no false negatives).
    The "\\x00" separator is non-alphanumeric, which keeps the phone-number
    lookaround markers matchable at part boundaries. Markers that only arise
    ACROSS a boundary are false positives, which merely route that message to
    the exact per-string pass below, never weaker redaction.
    """
    ...
    for message in messages:
        parts: list = []
        _collect_strings(message, parts)
        if not parts or not _might_contain_sensitive_text("\x00".join(parts)):
            out.append(message)
        else:
            out.append(_redact_value(message, _enabled=_enabled))
    return out
...
def redact_session_data(session_dict: dict) -> dict:
    ...
    if 'messages' in result:
        result['messages'] = _redact_message_list(
            result['messages'], _enabled=_enabled
        )
