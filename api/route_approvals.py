...
def approval_required_approver(session_key: str, approval_id: str = "") -> str:
    """Return the required approver for a pending governance approval.

    Empty means the approval is a normal Hermes approval with no identity lock.
    Reads both the WebUI polling queue and the live gateway queue under the
    shared approval lock. An explicit stale id never falls back to another
    queue entry.
    """
    ...
    with _lock:
        ...
        for entry in entries:
            ...
            if approval_id and entry.get("approval_id") != approval_id:
                continue
            required = str(entry.get("required_approver") or "").strip().lower()
            if required:
                return required
            if approval_id:
                return ""
        ...
        for gateway_entry in gateway_queue:
            data = getattr(gateway_entry, "data", None) or {}
            # Gateway entries have no WebUI approval_id until mirrored. With an
            # explicit id, only accept the entry whose mirror token matched in
            # the polling queue above; never bind a stale id to the live head.
            if approval_id:
                continue
            required = str(data.get("required_approver") or "").strip().lower()
            if required:
                return required
            break
    return ""
