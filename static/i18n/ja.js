...
window.__registerHermesLocale('ja', {
    ...
    mcp_restart_hint: 'ここでのサーバー変更は現状読み取り専用です。反映するには config.yaml を編集して SynthPulse を再起動してください。',
    ...
    approval_gateway_unsupported: '承認を実行するには新しいゲートウェイが必要です。有効にするには接続されている SynthPulse ゲートウェイをアップグレードしてください。',
    ...
    cmd_skills: '利用可能な SynthPulse スキルを一覧表示',
  ...
  status_agent_running:'エージェント実行中',
    ...
    status_hermes_home: 'SynthPulse ホーム',
    status_started: '開始',
    status_tokens: 'トークン',
    status_updated: '更新',
  ...
  untitled:'無題',
    ...
    session_title_regenerate_desc: '会話から新しいタイトルを生成するよう SynthPulse に依頼します',
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    settings_section_preferences_meta: 'SynthPulse Control のデフォルトと UI 動作。',
    ...
    settings_help_meta: 'SynthPulse Control のリソースとサポート。',
    settings_help_docs_label: 'ドキュメント',
    settings_help_docs_desc: 'ガイド、設定リファレンス、および SynthPulse Control README の全文。',
    ...
    settings_aux_advanced_timeout_desc: 'この補助タスクのリクエストタイムアウト時間。空欄の場合は SynthPulse のデフォルト値が適用されます。',
    ...
    kanban_work_queue_hint: 'これは SynthPulse Agent のワークキューです。タスクを作成またはトリアージし、担当者を割り当て、Ready に移動すると、ディスパッチャーがそれをクレームします。',
    ...
    kanban_assignee_profiles_label: 'SynthPulse プロファイル',
    ...
    tab_todos: 'ToDo',
    tab_insights: 'インサイト',
    tab_dashboard: 'SynthPulse ダッシュボード',
    ...
    disable_auth: '認証を無効化',
    sign_out: 'サインアウト',
    settings_label_shutdown: 'SynthPulse Control サーバーを停止',
    ...
    settings_shutdown_confirm_title: 'SynthPulse Control を停止',
    settings_shutdown_confirm_message: 'SynthPulse Control サーバーを停止しますか？',
    ...
    onboarding_lead: 'クイックガイドで SynthPulse を確認し、プロバイダ設定を保存し、ワークスペースとモデルを選択し、必要ならパスワードでアプリを保護します。',
    onboarding_back: '戻る',
    onboarding_continue: '続ける',
    onboarding_skip: 'セットアップをスキップ',
    onboarding_skipped: 'セットアップをスキップ — 既存設定を使用します。',
    onboarding_open: 'SynthPulse を開く',
    onboarding_step_system_title: 'システム確認',
    onboarding_step_system_desc: 'SynthPulse Agent と設定の可視性を確認します。',
    onboarding_step_setup_title: 'プロバイダ設定',
    onboarding_step_setup_desc: '最低限の SynthPulse プロバイダ設定を保存します。',
    ...
    onboarding_notice_system_ready: 'Web UI から SynthPulse Agent へ到達できそうです。',
    onboarding_notice_system_unavailable: 'SynthPulse Agent はまだ完全には利用できません。Bootstrap でインストールできますが、プロバイダ設定にはターミナルが必要な場合があります。',
    ...
    onboarding_notice_setup_already_ready: '動作する SynthPulse プロバイダ設定が既に検出されています。維持するか、ここで置き換えできます。',
    ...
    onboarding_api_key_help_prefix: 'SynthPulse の .env ファイルにシークレットとして保存されます — 使用変数:',
    ...
    onboarding_workspace_help: 'セットアップ完了後の新規チャットで SynthPulse が使用するモデルを選択してください。',
