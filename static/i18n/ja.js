...
window.__registerHermesLocale('ja', {
    offline_title: '接続が切断されました',
    offline_browser_detail: 'ブラウザはこのデバイスがオフラインだと報告しています。',
    offline_network_detail: '現在、このブラウザからSynthPulseに到達できません。',
    offline_autorefresh: 'SynthPulseに再接続できたら、このページを自動的に更新します。',
    ...
    mcp_restart_hint: 'ここでのサーバー変更は現状読み取り専用です。反映するには config.yaml を編集して SynPulse を再起動してください。',
    ...
    approval_gateway_unsupported: '承認を実行するには新しいゲートウェイが必要です。有効にするには接続されている SynPulse ゲートウェイをアップグレードしてください。',
    ...
    provider_mismatch_warning: (m,p)=>`"${m}" は設定されたプロバイダ (${p}) で動作しない可能性があります。このまま送信するか、ターミナルで \`model picker\` を実行して切り替えてください。`,
    ...
    cmd_skills: '利用可能な SynPulse スキルを一覧表示',
  ...
  status_agent_running:'エージェント実行中',
    ...
    status_hermes_home: 'SynPulse ホーム',
    status_started: '開始',
    status_tokens: 'トークン',
    status_updated: '更新',
  ...
  usage_load_failed:'使用量読み込み失敗: ',
    ...
    chat_mode: 'チャットモード',
    chat_mode_super: 'スーパーエージェント',
    chat_mode_normal: '通常のチャット',
    chat_mode_super_title: 'ツールとスキルにフルアクセス',
    chat_mode_normal_title: 'ツールを絞って応答を高速化',
    chat_mode_switched: 'チャットモードを変更しました',
    chat_mode_failed: 'チャットモードを変更できませんでした: ',
    session_toolsets_failed:'ツールセット更新失敗: ',
  untitled:'無題',
    no_personalities: 'パーソナリティが見つかりません (the runtime personalities directory に追加してください)',
    ...
    session_title_regenerate_desc: '会話から新しいタイトルを生成するよう SynPulse に依頼します',
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    settings_section_preferences_meta: 'SynPulse Control のデフォルトと UI 動作。',
    ...
    composer_control_chat_mode: 'チャットモード',
    ...
    settings_plugins_meta: 'インストール済みのSynthPulseプラグインと登録されているライフサイクルフックを確認できます。このパネルは読み取り専用です。',
    settings_plugins_empty: '現在表示できるSynthPulseプラグインがありません。agent CLI/configからプラグインをインストールまたは有効化すると、ここに表示されます。',
    ...
    settings_help_meta: 'SynPulse Control のリソースとサポート。',
    settings_help_docs_label: 'ドキュメント',
    settings_help_docs_desc: 'ガイド、設定リファレンス、および SynPulse Control README の全文。',
    ...
    settings_aux_advanced_timeout_desc: 'この補助タスクのリクエストタイムアウト時間。空欄の場合は SynPulse のデフォルト値が適用されます。',
    ...
    settings_label_dashboard_mode: '公式SynthPulseダッシュボード',
    settings_desc_dashboard_mode: '公式synthpulseダッシュボードが到達可能な場合にナビゲーションレールにリンクを表示します。公開リバースプロキシURLはブラウザのみのリンクとして保存され、サーバーによるプローブは行われません。',
    ...
    settings_desc_gateway_status: 'SynthPulseゲートウェイのステータス（Telegram、Discord、Slackなど）',
    ...
    kanban_work_queue_hint: 'これは SynPulse Agent のワークキューです。タスクを作成またはトリアージし、担当者を割り当て、Ready に移動すると、ディスパッチャーがそれをクレームします。',
    ...
    kanban_assignee_profiles_label: 'SynPulse プロファイル',
    ...
    tab_todos: 'ToDo',
    tab_insights: 'インサイト',
    tab_dashboard: 'SynPulse ダッシュボード',
    ...
    insights_skill_usage_footer: 'データ元: the runtime skills directory',
    ...
    settings_desc_sync_insights: 'WebUI のトークン使用量を state.db にミラーし、/insights にブラウザセッションのデータを含めます。デフォルトはオフ。',
    ...
    password_env_var_locked: '現在 password environment variable 環境変数が設定されており優先されます。ここで管理するには変数を解除してサーバーを再起動してください。',
    password_env_var_locked_placeholder: 'ロック中: password environment variable 環境変数が設定されています',
    disable_auth: '認証を無効化',
    sign_out: 'サインアウト',
    settings_label_shutdown: 'SynPulse Control サーバーを停止',
    ...
    settings_shutdown_confirm_title: 'SynPulse Control を停止',
    settings_shutdown_confirm_message: 'SynPulse Control サーバーを停止しますか？',
    ...
    providers_oauth_config_yaml_hint: 'config.yaml でトークンが設定されています。更新するには config.yaml の providers セクションを編集するか agent auth を実行してください。',
    providers_oauth_not_configured_hint: '未認証です。ターミナルで agent auth を実行してこのプロバイダを設定してください。',
    providers_save: '保存',
    ...
    onboarding_title: 'SynPulse Control へようこそ',
    onboarding_lead: 'クイックガイドで SynPulse を確認し、プロバイダ設定を保存し、ワークスペースとモデルを選択し、必要ならパスワードでアプリを保護します。',
    onboarding_back: '戻る',
    onboarding_continue: '続ける',
    onboarding_skip: 'セットアップをスキップ',
    onboarding_skipped: 'セットアップをスキップ — 既存設定を使用します。',
    onboarding_open: 'SynPulse を開く',
    onboarding_step_system_title: 'システム確認',
    onboarding_step_system_desc: 'SynPulse Agent と設定の可視性を確認します。',
    onboarding_step_setup_title: 'プロバイダ設定',
    onboarding_step_setup_desc: '最低限の SynPulse プロバイダ設定を保存します。',
    ...
    onboarding_notice_system_ready: 'Web UI から SynPulse Agent へ到達できそうです。',
    onboarding_notice_system_unavailable: 'SynPulse Agent はまだ完全には利用できません。Bootstrap でインストールできますが、プロバイダ設定にはターミナルが必要な場合があります。',
    ...
    onboarding_notice_setup_required: 'シンプルなプロバイダ経路をここで選んでください。高度な OAuth フローは現状 agent CLI 側で扱います。',
    onboarding_notice_setup_already_ready: '動作する SynPulse プロバイダ設定が既に検出されています。維持するか、ここで置き換えできます。',
    onboarding_oauth_provider_ready_title: 'プロバイダは認証済み',
    onboarding_oauth_provider_ready_body: 'このインスタンスは agent CLI で設定された OAuth プロバイダ (<strong>{provider}</strong>) を使用します。APIキーは不要 — 続けるをクリックしてセットアップを完了してください。',
    onboarding_oauth_provider_not_ready_title: 'OAuth プロバイダは未認証',
    onboarding_oauth_provider_not_ready_body: 'このインスタンスは <strong>{provider}</strong> を使用しますが、これは APIキー ではなく OAuth を使用します。ターミナルで the agent auth command または the model picker を実行して認証し、Web UI を再読み込みしてください。',
    ...
    onboarding_api_key_help_prefix: 'SynPulse の .env ファイルにシークレットとして保存されます — 使用変数:',
    ...
    onboarding_workspace_help: 'セットアップ完了後の新規チャットで SynPulse が使用するモデルを選択してください。',
    ...
    cron_script_path_hint: '絶対パスでない限り the runtime scripts directory から解決されます。動作を変えるにはサーバー上のスクリプトを編集してください。',
