...
window.__registerHermesLocale('ru', {
    offline_title: 'Соединение потеряно',
    offline_browser_detail: 'Браузер сообщает, что это устройство офлайн.',
    offline_network_detail: 'SynPulse сейчас недоступен из этого браузера.',
    offline_autorefresh: 'Я автоматически обновлю страницу, когда SynPulse снова станет доступен.',
    ...
    `"${m}" может не работать с вашим настроенным провайдером (${p}). Всё равно отправить или запустите \`model picker\` в терминале, чтобы переключиться.`,
    ...
    cmd_skills: 'Показать доступные навыки SynPulse',
    ...
    no_personalities: 'Личности не найдены (добавьте их в the runtime personalities directory)',
    ...
    session_title_regenerate_desc: 'Попросить SynPulse сгенерировать новое название на основе беседы',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    settings_label_dashboard_mode: 'Официальная панель SynPulse',
    settings_desc_dashboard_mode: 'Показывать ссылку в навигационной панели, когда официальная панель synthpulse доступна. URL публичного обратного прокси хранятся только как ссылки в браузере и никогда не проверяются сервером.',
    ...
    settings_desc_gateway_status: 'Статус шлюза SynPulse (Telegram, Discord, Slack и т.д.)',
    ...
    kanban_assignee_profiles_label: 'Профили SynPulse',
    ...
    settings_desc_sync_insights: 'Синхронизирует использование токенов WebUI в state.db, чтобы /insights включал данные браузерных сеансов. Выключено по умолчанию.',
    ...
    password_env_var_locked: 'Переменная окружения password environment variable сейчас задана и имеет приоритет. Сбросьте её и перезапустите сервер, чтобы управлять паролем отсюда.',
    password_env_var_locked_placeholder: 'Заблокировано: задана переменная password environment variable',
    disable_auth: 'Отключить авторизацию',
    sign_out: 'Выйти',
    settings_label_shutdown: 'Остановить сервер SynPulse Control',
    ...
    settings_shutdown_confirm_title: 'Остановить SynPulse Control',
    settings_shutdown_confirm_message: 'Остановить сервер SynPulse Control?',
    ...
    onboarding_title: 'Добро пожаловать в SynPulse Control',
    onboarding_lead: 'Краткая пошаговая настройка проверит SynPulse, сохранит рабочую конфигурацию провайдера, выберет рабочее пространство и модель и при желании защитит приложение паролем.',
    ...
    onboarding_open: 'Открыть SynPulse',
    onboarding_step_system_title: 'Проверка системы',
    onboarding_step_system_desc: 'Проверить SynPulse Agent и видимость конфигурации.',
    onboarding_step_setup_title: 'Настройка провайдера',
    onboarding_step_setup_desc: 'Сохранить минимальную рабочую конфигурацию провайдера SynPulse.',
    ...
    onboarding_notice_system_ready: 'SynPulse Agent, похоже, доступен из Web UI.',
    onboarding_notice_system_unavailable: 'SynPulse Agent ещё не полностью доступен. Bootstrap может установить его, но для настройки провайдера всё ещё может понадобиться терминал.',
    ...
    onboarding_notice_setup_required: 'Выберите здесь простой путь настройки провайдера. Продвинутые OAuth-сценарии пока остаются в agent CLI.',
    onboarding_notice_setup_already_ready: 'Уже обнаружена рабочая конфигурация провайдера SynPulse. Вы можете оставить её или заменить здесь.',
    onboarding_oauth_provider_ready_title: 'Провайдер уже авторизован',
    onboarding_oauth_provider_ready_body: 'Этот экземпляр настроен на использование OAuth-провайдера (<strong>{provider}</strong>), настроенного через agent CLI. API-ключ здесь не нужен — нажмите «Продолжить», чтобы завершить настройку.',
    onboarding_oauth_provider_not_ready_title: 'OAuth-провайдер ещё не авторизован',
    onboarding_oauth_provider_not_ready_body: 'Этот экземпляр настроен на использование <strong>{provider}</strong>, который работает через OAuth, а не через API-ключ. Запустите the agent auth command или the model picker в терминале, чтобы пройти авторизацию, затем обновите Web UI.',
    ...
    onboarding_api_key_help_prefix: 'Сохраняется как секрет в вашем файле `.env` SynPulse с помощью',
    ...
    onboarding_workspace_help: 'Выберите модель, которую SynPulse должен использовать для новых чатов после завершения настройки.',
    ...
    cron_script_path_hint: 'Ищется в the runtime scripts directory, если не указан абсолютный путь. Логику меняй в файле скрипта на сервере.',
    ...
    settings_desc_voice_mode: 'Показывать кнопку голосового режима (аудиоволны) рядом с микрофоном диктовки. SynPulse автоматически отправляет реплики после паузы и зачитывает ответы вслух. Требуется браузер с поддержкой распознавания речи и TTS.',
