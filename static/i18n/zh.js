...
window.__registerHermesLocale('zh', {
    offline_title: '连接已断开',
    offline_browser_detail: '浏览器报告此设备当前离线。',
    offline_network_detail: '此浏览器当前无法连接到 SynthPulse。',
    offline_autorefresh: '当 SynthPulse 可访问时，我会自动刷新此页面。',
    ...
    mic_insecure_origin: '语音输入需要安全连接。请通过 HTTPS 协议或从本地主机打开SynthPulse，以便使用麦克风。',
    ...
    mcp_restart_hint: '服务器更改目前为只读。请编辑 config.yaml 并重启 SynthPulse。',
    ...
    approval_gateway_unsupported: '批准操作需要更新版本的网关。请升级已连接的SynthPulse网关以实现此功能。',
    ...
    provider_mismatch_warning: (m,p)=>`\"${m}\" 可能无法在当前配置的提供商 (${p}) 下工作。直接发送，或在终端运行 \`model picker\` 切换。`,
    ...
    chat_mode: '聊天模式',
    chat_mode_super: '超级智能体',
    chat_mode_normal: '普通聊天',
    chat_mode_super_title: '完整的工具和技能访问',
    chat_mode_normal_title: '工具更少，回复更快',
    chat_mode_switched: '聊天模式已更新',
    chat_mode_failed: '无法更改聊天模式：',
    ...
    settings_plugins_meta: '查看已安装的 SynthPulse 插件及其注册的生命周期挂钩。此面板为只读。',
    settings_plugins_empty: '当前没有可见的 SynthPulse 插件。通过 agent CLI/配置文件安装或启用插件后即可在此查看。',
    ...
    no_personalities: '没有找到人设（可添加到 the runtime personalities directory）',
    ...
    session_title_regenerate_desc: '\u8ba9 SynthPulse \u6839\u636e\u5bf9\u8bdd\u751f\u6210\u65b0\u7684\u6807\u9898',
  ...
  folder_add_as_space_title: '添加为工作区？',
    ...
    settings_aux_advanced_timeout_desc: '此辅助任务的请求超时时间。留空则使用SynthPulse默认设置。',
    ...
    settings_label_dashboard_mode: '官方 SynthPulse 仪表盘',
    settings_desc_dashboard_mode: '当官方 synthpulse 仪表盘可访问时，在导航栏显示链接。公开反向代理 URL 仅作为浏览器链接存储，不会由服务器探测。',
    ...
    settings_desc_gateway_status: 'SynthPulse 网关状态（Telegram、Discord、Slack 等）',
    ...
    kanban_work_queue_hint: '这是 SynthPulse Agent 的工作队列。创建或分类任务，分配任务，将其移至”就绪”状态，然后让调度员认领任务。',
    ...
    kanban_assignee_profiles_label: 'SynthPulse 配置',
    ...
    sign_out: '退出登录',
    settings_label_shutdown: '停止 SynthPulse Control 服务器',
    ...
    settings_shutdown_confirm_message: '停止 SynthPulse Control 服务器？',
    ...
    providers_oauth_config_yaml_hint: '通过 config.yaml 配置的令牌。如需更新，请编辑 config.yaml 中的 providers 部分或运行 agent auth。',
    providers_oauth_not_configured_hint: '未认证。在终端中运行 agent auth 以配置此提供商。',
    providers_save: '保存',
    ...
    password_env_var_locked: '当前已设置 password environment variable 环境变量并具有优先级。请取消该变量并重启服务器，才能在此管理密码。',
    password_env_var_locked_placeholder: '已锁定：已设置 password environment variable 环境变量',
    disable_auth: '停用认证',
    ...
    onboarding_lead: '快速引导将验证 SynthPulse、保存真实的提供商配置、选择工作区和模型，并可选设置密码保护应用。',
    onboarding_back: '返回',
    onboarding_continue: '继续',
    onboarding_skip: '跳过设置',
    onboarding_skipped: '设置已跳过 — 使用现有配置。',
    onboarding_open: '打开 SynthPulse',
    onboarding_step_system_title: '系统检查',
    onboarding_step_system_desc: '验证 SynthPulse Agent 与配置可见性。',
    onboarding_step_setup_title: '提供商设置',
    onboarding_step_setup_desc: '保存最小可用的 SynthPulse 提供商配置。',
    ...
    onboarding_notice_system_ready: 'SynthPulse Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'SynthPulse Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    ...
    onboarding_notice_setup_required: '请先在此选择一个简单的提供商路径。高级 OAuth 流程暂时仍建议在 agent CLI 中完成。',
    onboarding_notice_setup_already_ready: '已检测到可用的 SynthPulse 提供商配置。你可以保留它，或在这里替换。',
    onboarding_oauth_provider_ready_title: '提供商已完成认证',
    onboarding_oauth_provider_ready_body: '此实例已配置为使用通过 agent CLI 设置的 OAuth 提供商（<strong>{provider}</strong>）。这里不需要 API key，点击继续即可完成设置。',
    onboarding_oauth_provider_not_ready_title: 'OAuth 提供商尚未认证',
    onboarding_oauth_provider_not_ready_body: '此实例已配置为使用 <strong>{provider}</strong>，该提供商使用 OAuth 而非 API key。请在终端运行 the agent auth command 或 the model picker 完成认证，然后重新加载 Web UI。',
    ...
    onboarding_api_key_help_prefix: '会作为密钥保存到 SynthPulse .env 文件中，变量名为',
    ...
    onboarding_workspace_help: '选择设置完成后 SynthPulse 在新聊天中使用的模型。',
    ...
    cron_script_path_hint: '除非是绝对路径，否则在 the runtime scripts directory 下解析。请在服务器上编辑脚本文件以更改行为。',
    ...
    settings_section_preferences_meta: 'SynthPulse Control 的默认和 UI 行为。',
    ...
    settings_help_meta: 'SynthPulse Control的资源与支持 ',
    settings_help_docs_label: '文档',
    settings_help_docs_desc: '指南、配置参考以及完整的SynthPulse Control自述文件。',
    ...
    status_profile: '配置文件',
    status_hermes_home: 'SynthPulse 主目录',
    status_started: '开始时间',
    status_tokens: '令牌',
    status_unknown: '未知',
    cmd_yolo: 'YOLO 模式切换',
    ...
    settings_desc_voice_mode: '在听写麦克风旁显示语音模式按钮（音频波形）。让您自然说话 — SynthPulse 会在停顿后自动发送并朗读回复。需要支持语音识别和 TTS 的浏览器。',
    ...
    insights_skill_usage_footer: '数据来源于 the runtime skills directory',
