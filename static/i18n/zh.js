...
window.__registerHermesLocale('zh', {
    ...
    offline_network_detail: '此浏览器当前无法连接到 SynPulse。',
    offline_autorefresh: '当 SynPulse 可访问时，我会自动刷新此页面。',
    ...
    mcp_restart_hint: '服务器更改目前为只读。请编辑 config.yaml 并重启 SynPulse。',
    ...
    settings_plugins_meta: '查看已安装的 SynPulse 插件及其注册的生命周期挂钩。此面板为只读。',
    settings_plugins_empty: '当前没有可见的 SynPulse 插件。通过 agent CLI/配置文件安装或启用插件后即可在此查看。',
    ...
    session_title_regenerate_desc: '\u8ba9 SynPulse \u6839\u636e\u5bf9\u8bdd\u751f\u6210\u65b0\u7684\u6807\u9898',
  ...
  folder_add_as_space_title: '添加为工作区？',
    ...
    settings_label_dashboard_mode: '官方 SynPulse 仪表盘',
    ...
    settings_desc_gateway_status: 'SynPulse 网关状态（Telegram、Discord、Slack 等）',
    ...
    kanban_work_queue_hint: '这是 SynPulse Agent 的工作队列。创建或分类任务，分配任务，将其移至”就绪”状态，然后让调度员认领任务。',
    ...
    kanban_assignee_profiles_label: 'SynPulse 配置',
    ...
    sign_out: '退出登录',
    settings_label_shutdown: '停止 SynPulse Control 服务器',
    ...
    settings_shutdown_confirm_message: '停止 SynPulse Control 服务器？',
    ...
    onboarding_title: '欢迎使用 SynPulse Control',
    archive_extracted: (n, c) => `从 ${c} 个压缩包中解压了 ${n} 个文件`,
    onboarding_lead: '快速引导将验证 SynPulse、保存真实的提供商配置、选择工作区和模型，并可选设置密码保护应用。',
    onboarding_back: '返回',
    onboarding_continue: '继续',
    onboarding_skip: '跳过设置',
    onboarding_skipped: '设置已跳过 — 使用现有配置。',
    onboarding_open: '打开 SynPulse',
    onboarding_step_system_title: '系统检查',
    onboarding_step_system_desc: '验证 SynPulse Agent 与配置可见性。',
    onboarding_step_setup_title: '提供商设置',
    onboarding_step_setup_desc: '保存最小可用的 SynPulse 提供商配置。',
    ...
    onboarding_notice_system_ready: 'SynPulse Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'SynPulse Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    ...
    onboarding_notice_setup_already_ready: '已检测到可用的 SynPulse 提供商配置。你可以保留它，或在这里替换。',
    ...
    onboarding_api_key_help_prefix: '会作为密钥保存到 SynPulse .env 文件中，变量名为',
    ...
    onboarding_workspace_help: '选择设置完成后 SynPulse 在新聊天中使用的模型。',
    ...
    settings_section_preferences_meta: 'SynPulse Control 的默认和 UI 行为。',
    ...
    settings_help_meta: 'SynPulse Control的资源与支持 ',
    ...
    status_profile: '配置文件',
    status_hermes_home: 'SynPulse 主目录',
    status_started: '开始时间',
    status_tokens: '令牌',
    status_unknown: '未知',
    cmd_yolo: 'YOLO 模式切换',
    ...
    settings_desc_voice_mode: '在听写麦克风旁显示语音模式按钮（音频波形）。让您自然说话 — SynPulse 会在停顿后自动发送并朗读回复。需要支持语音识别和 TTS 的浏览器。',
