...
window.__registerHermesLocale('zh-Hant', {

    offline_title: '連線中斷',
    offline_browser_detail: '瀏覽器回報此裝置目前離線。',
    offline_network_detail: 'SynPulse 目前無法從這個瀏覽器連上。',
    offline_autorefresh: 'SynPulse 重新連線後，系統會自動重新整理此頁面。',
    ...
    mic_insecure_origin: '語音輸入需要安全連線。請透過 HTTPS 或 localhost 開啟 SynPulse 才能使用麥克風。',
    ...
    mcp_restart_hint: '伺服器變更目前在此為唯讀。請編輯 config.yaml 並重新啟動 SynPulse，讓變更生效。',
    ...
    provider_mismatch_warning: (m,p)=>`「${m}」可能無法搭配已設定的供應商（${p}）使用。仍要送出，或在終端機執行 \`model picker\` 來切換。`,
    ...
    status_profile: '個人資料',
    status_hermes_home: 'SynPulse 主目錄',
    status_started: '開始時間',
    status_updated: '已更新',
    status_tokens: 'Token',
    ...
    chat_mode: '聊天模式',
    chat_mode_super: '超級智慧體',
    chat_mode_normal: '一般聊天',
    chat_mode_super_title: '完整的工具與技能存取',
    chat_mode_normal_title: '工具更少，回覆更快',
    chat_mode_switched: '聊天模式已更新',
    chat_mode_failed: '無法變更聊天模式：',
    session_toolsets_failed: '更新工具集失敗：',
    untitled: '未命名',
    no_personalities: '找不到角色（可新增至 the runtime personalities directory）',
    ...
    session_title_regenerate_desc: '請 SynPulse 依對話產生新的標題',
    ...
    settings_section_preferences_meta: 'SynPulse Control 的預設值與介面行為。',
    ...
    composer_control_chat_mode: '聊天模式',
    ...
    settings_plugins_meta: '檢視已安裝的 SynPulse 外掛及其註冊的生命週期掛鉤。此面板為唯讀。',
    settings_plugins_empty: '目前沒有可見的 SynPulse 外掛。透過 agent CLI/設定檔安裝或啟用外掛後即可在此檢視。',
    ...
    settings_label_dashboard_mode: '官方 SynPulse 儀表板',
    settings_desc_dashboard_mode: '當官方 synthpulse 儀表板可存取時，在導航欄顯示連結。公開反向代理 URL 僅作為瀏覽器連結儲存，不會由伺服器探測。',
    ...
    settings_desc_gateway_status: 'SynPulse 閘道狀態（Telegram、Discord、Slack 等）',
    ...
    kanban_work_queue_hint: '這是 SynPulse Agent 的工作佇列。建立或分類任務、指派對象、移至 Ready，然後讓調度器認領。',
    ...
    kanban_assignee_profiles_label: 'SynPulse 設定檔',
    ...
    insights_skill_usage_footer: '資料來源：the runtime skills directory',
    ...
    settings_desc_voice_mode: '在聽寫麥克風旁顯示語音模式按鈕（音訊波形）。可自然說話，SynPulse 會在停頓後自動送出並朗讀回覆。需要同時支援語音辨識與 TTS 的瀏覽器。',
    ...
    settings_desc_sync_insights: '將 WebUI token 用量鏡射到 state.db，讓 /insights 納入瀏覽器工作階段資料。預設為關閉。',
    ...
    settings_desc_bot_name: '助理在整個 UI 中顯示的名稱。預設為 SynPulse。',
    settings_desc_password: '輸入新密碼即可設定或變更密碼。留空會保留目前設定。',
    password_placeholder: '輸入新密碼……',
    password_env_var_locked: '目前已設定 password environment variable 環境變數且優先生效。請取消設定並重新啟動伺服器，才能在此管理密碼。',
    password_env_var_locked_placeholder: '已鎖定：已設定 password environment variable 環境變數',
    disable_auth: '停用驗證',
    sign_out: '登出',
    settings_label_shutdown: '停止 SynPulse Control 伺服器',
    ...
    settings_shutdown_confirm_title: '停止 SynPulse Control',
    settings_shutdown_confirm_message: '停止 SynPulse Control 伺服器？',
    ...
    providers_oauth_config_yaml_hint: '已透過 config.yaml 設定權證。要更新，請編輯 config.yaml 中的 providers 區段，或執行 agent auth。',
    providers_oauth_not_configured_hint: '尚未驗證。請在終端機執行 agent auth 以設定此提供者。',
    providers_save: '儲存',
    ...
    onboarding_title: '歡迎使用 SynPulse Control',
    onboarding_lead: '快速設定精靈會驗證 SynPulse、儲存實際的供應商設定、選擇工作區與模型，並可選擇用密碼保護應用程式。',
    onboarding_back: '上一頁',
    onboarding_continue: '繼續',
    onboarding_skip: '跳過設定',
    onboarding_skipped: '已略過設定，使用現有設定。',
    onboarding_open: '開啟 SynPulse',
    onboarding_step_system_title: '系統檢查',
    onboarding_step_system_desc: '驗證 SynPulse Agent 與設定可見性。',
    onboarding_step_setup_title: '供應商設定',
    onboarding_step_setup_desc: '儲存最低限度的 SynPulse 供應商設定。',
    ...
    onboarding_notice_system_ready: 'SynPulse Agent 看起來可由 Web UI 存取。',
    onboarding_notice_system_unavailable: 'SynPulse Agent 尚未完全可用。Bootstrap 可以安裝它，但供應商設定可能仍需要使用終端機。',
    ...
    onboarding_notice_setup_required: '請在這裡選擇簡易供應商路徑。進階 OAuth 流程目前仍需使用 agent CLI 處理。',
    onboarding_notice_setup_already_ready: '已偵測到可用的 SynPulse 供應商設定。可以保留或在這裡替換。',
    onboarding_oauth_provider_ready_title: '供應商已完成驗證',
    onboarding_oauth_provider_ready_body: '這個執行個體已設定為使用 agent CLI 設定的 OAuth 供應商（<strong>{provider}</strong>）。這裡不需要 API 金鑰，請點選 [繼續] 完成設定。',
    onboarding_oauth_provider_not_ready_title: 'OAuth 供應商尚未完成驗證',
    onboarding_oauth_provider_not_ready_body: '這個執行個體已設定為使用 <strong>{provider}</strong>，此供應商使用 OAuth 而不是 API 金鑰。請在終端機執行 the agent auth command 或 the model picker 完成驗證，然後重新載入 Web UI。',
    ...
    onboarding_api_key_help_prefix: '系統會使用以下名稱，將它以祕密值形式儲存在 SynPulse .env 檔案中',
    ...
    onboarding_workspace_help: '選擇設定完成後 SynPulse 應在新聊天中使用的模型。',
    ...
    cron_script_path_hint: '除非為絕對路徑，否則會在 the runtime scripts directory 下解析。請在伺服器上編輯腳本檔以變更行為。',
