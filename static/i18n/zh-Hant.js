...
window.__registerHermesLocale('zh-Hant', {
    ...
    offline_network_detail: 'SynthPulse 目前無法從這個瀏覽器連上。',
    offline_autorefresh: 'SynthPulse 重新連線後，系統會自動重新整理此頁面。',
    ...
    mic_insecure_origin: '語音輸入需要安全連線。請透過 HTTPS 或 localhost 開啟 SynthPulse 才能使用麥克風。',
    ...
    mcp_restart_hint: '伺服器變更目前在此為唯讀。請編輯 config.yaml 並重新啟動 SynthPulse，讓變更生效。',
    ...
    cmd_skills: '列出可用的 SynthPulse 技能',
    ...
    status_profile: '個人資料',
    status_hermes_home: 'SynthPulse 主目錄',
    status_started: '開始時間',
    status_updated: '已更新',
    status_tokens: 'Token',
    ...
    session_title_regenerate_desc: '請 SynthPulse 依對話產生新的標題',
    ...
    settings_section_preferences_meta: 'SynthPulse Control 的預設值與介面行為。',
    ...
    settings_plugins_meta: '檢視已安裝的 SynthPulse 外掛及其註冊的生命週期掛鉤。此面板為唯讀。',
    settings_plugins_empty: '目前沒有可見的 SynthPulse 外掛。透過 agent CLI/設定檔安裝或啟用外掛後即可在此檢視。',
    ...
    settings_label_dashboard_mode: '官方 SynthPulse 儀表板',
    ...
    settings_desc_gateway_status: 'SynthPulse 閘道狀態（Telegram、Discord、Slack 等）',
    ...
    kanban_work_queue_hint: '這是 SynthPulse Agent 的工作佇列。建立或分類任務、指派對象、移至 Ready，然後讓調度器認領。',
    ...
    kanban_assignee_profiles_label: 'SynthPulse 設定檔',
    ...
    settings_desc_voice_mode: '在聽寫麥克風旁顯示語音模式按鈕（音訊波形）。可自然說話，SynthPulse 會在停頓後自動送出並朗讀回覆。需要同時支援語音辨識與 TTS 的瀏覽器。',
    ...
    settings_desc_bot_name: '助理在整個 UI 中顯示的名稱。預設為 SynthPulse。',
    ...
    disable_auth: '停用驗證',
    sign_out: '登出',
    settings_label_shutdown: '停止 SynthPulse Control 伺服器',
    ...
    settings_shutdown_confirm_title: '停止 SynthPulse Control',
    settings_shutdown_confirm_message: '停止 SynthPulse Control 伺服器？',
    ...
    onboarding_lead: '快速設定精靈會驗證 SynthPulse、儲存實際的供應商設定、選擇工作區與模型，並可選擇用密碼保護應用程式。',
    onboarding_back: '上一頁',
    onboarding_continue: '繼續',
    onboarding_skip: '跳過設定',
    onboarding_skipped: '已略過設定，使用現有設定。',
    onboarding_open: '開啟 SynthPulse',
    onboarding_step_system_title: '系統檢查',
    onboarding_step_system_desc: '驗證 SynthPulse Agent 與設定可見性。',
    onboarding_step_setup_title: '供應商設定',
    onboarding_step_setup_desc: '儲存最低限度的 SynthPulse 供應商設定。',
    ...
    onboarding_notice_system_ready: 'SynthPulse Agent 看起來可由 Web UI 存取。',
    onboarding_notice_system_unavailable: 'SynthPulse Agent 尚未完全可用。Bootstrap 可以安裝它，但供應商設定可能仍需要使用終端機。',
    ...
    onboarding_notice_setup_already_ready: '已偵測到可用的 SynthPulse 供應商設定。可以保留或在這裡替換。',
    ...
    onboarding_api_key_help_prefix: '系統會使用以下名稱，將它以祕密值形式儲存在 SynthPulse .env 檔案中',
    ...
    onboarding_workspace_help: '選擇設定完成後 SynthPulse 應在新聊天中使用的模型。',
