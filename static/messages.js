    ...
    source.addEventListener('apperror',e=>{
      ...
      if(S.session&&eventMatchesCurrent){
        ...
        try{
          ...
          const isOverloaded=d.type==='overloaded';
          const isServiceUnavailable=d.type==='service_unavailable';
          ...
          const label=isCancelled?'Task cancelled':isInterrupted?'Response interrupted':isCompressionExhausted?'Context compression exhausted':isToolLimitReached?'Tool iteration limit reached':isQuotaExhausted?'Out of credits':isRateLimit?'Rate limit reached':isOverloaded?'Service busy':isServiceUnavailable?'Service unavailable':isGatewayAuthError?(typeof t==='function'?t('gateway_auth_label'):'Gateway authentication failed'):isAuthMismatch?(typeof t==='function'?t('provider_mismatch_label'):'Provider mismatch'):isModelNotFound?(typeof t==='function'?t('model_not_found_label'):'Model not found'):isNoResponse?'No response from provider':'Error';
          const hint=d.hint?`\n\n*${d.hint}*`:'';
