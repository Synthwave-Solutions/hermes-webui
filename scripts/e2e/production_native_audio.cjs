...
(async()=>{
  try {
    ...
    hostVerified = true;
    process.umask(0o077);
    const deployment=JSON.parse(fs.readFileSync(root+'/qa-release-deployment-v4-20260909.json','utf8'));
    need(deployment.status==='deployed_pending_live_smoke' && deployment.gateway_drains_released && deployment.governance_policy_preserved, 'deployment_not_verified');
    const pid=Number(cp.execFileSync('systemctl',['--user','show','hermes-webui.service','--property=MainPID','--value'],{encoding:'utf8'}).trim());
    const expected=deployment.services_after_release['hermes-webui.service'];
    const startTicks=()=>{const raw=fs.readFileSync('/proc/'+pid+'/stat','utf8');return raw.slice(raw.lastIndexOf(')')+1).trim().split(/\s+/)[19];};
    need(pid===expected.pid && fs.statSync('/proc/'+pid).uid===1000 && startTicks()===String(expected.start_time), 'webui_instance_changed');
    report.operator_sha256=crypto.createHash('sha256').update(fs.readFileSync(__filename)).digest('hex');
    report.deployment_receipt='qa-release-deployment-v4-20260909.json';
    report.deployed_commits=deployment.new_commits;
    ...
    need(startTicks()===String(expected.start_time),'webui_instance_changed_during_environment_read');
    ...
    const phrase=/cobalt\s*river\s*(?:seven|7)/i;
    const targetUsers=userRows.filter({hasText:phrase}),targetReplies=assistantRows.filter({hasText:phrase});
    report.synthetic_capture_may_start_mid_loop=true;
    ...
    for(let turn=0;turn<2;turn++){
      const before={user:await targetUsers.count(),assistant:await targetReplies.count(),rtp:await rtpTotals()};
      await page.locator('#realtimeVoiceMic').click();
      await page.waitForFunction(n=>[...document.querySelectorAll('#realtimeVoiceTranscript [data-role=user]')].filter(e=>/cobalt\s*river\s*(?:seven|7)/i.test(e.textContent)).length>n,before.user,{timeout:55000});
      await page.locator('#realtimeVoiceMic').click();
      await page.waitForFunction(n=>[...document.querySelectorAll('#realtimeVoiceTranscript [data-role=assistant]')].filter(e=>/cobalt\s*river\s*(?:seven|7)/i.test(e.textContent)).length>n,before.assistant,{timeout:30000});
    ...
    report.expected_phrase_reply_count=await targetReplies.count();
    report.synthetic_captions=await page.locator('#realtimeVoiceTranscript .realtime-voice-transcript-row').evaluateAll(es=>es.map(e=>({role:e.dataset.role,text:e.textContent.slice(0,1000)})));
    ...
    if(hostVerified)fs.writeFileSync(root+'/qa-release-native-audio-v3-20260909.json',JSON.stringify(report,null,2),{flag:'wx',mode:0o600});
